...
也不在官方《支持的硬件平台》「一、已支持开发板」清单内，需要从板级 bring-up 做起，
符合本方向「选择一款尚未适配 openvela 的硬件平台，完成从底层 BSP 移植、驱动开发到
系统构建的全链路适配工作」的定义，以及「**已适配的开发板不计入本赛道**」的排除条件。

> **关于方向选择**：官方三个方向「**不作为独立赛道，仅为选题参考**，鼓励多方向组合开发」。
> 本作品**只申报「新硬件平台适配」一个方向**。
> 「AI 硬件产品创新」方向要求基于 openvela ＋ `ai_agent` 开发具备「主动＋执行」能力的
> 嵌入式 Agent 应用（含 `/data/agent/skills/` **运行时 Skill**），本作品未实现该方向的要求，
> 故不申报 —— 尽管本适配所服务的 SomatoSync 系统本身具备端侧 AI 能力（节点片内双 ESKF
> 姿态融合、网关侧 AI 康复分析），但那部分运行在**原有 ESP-IDF 原型**中，
> **尚未迁移到 openvela**，按该方向「不运行在 openvela 设备上的不算」的口径不计入。
